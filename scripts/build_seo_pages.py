...
def _skill_card(skill: dict) -> str:
    ...
    if isinstance(test_score, (int, float)):
        test_badge = f'<div class="freellm-test-strip tested"><strong>{_esc(test_score)}/10</strong><span>FreeLLM 实测 · {_esc(test_status)}</span><small>{_esc(test_level)}</small></div>'
    else:
        blocked = test_level == "preflight"
        test_badge = f'<div class="freellm-test-strip {"blocked" if blocked else "pending"}><strong>{"BLOCKED" if blocked else "TEST"}</strong><span>{_esc(test_status)}</span><small>{_esc(test_level)}</small></div>'
    return f'''<article class="skill-card" data-skill-id="{_esc(skill.get('id'))}" data-category="{_esc(skill.get('category'))}" data-status="{_esc(skill.get('status'))}">
      ...
      <h2>{_esc(skill.get('name'))}</h2>{test_badge}<p><span class="skill-description-zh" lang="zh-CN">{_esc(description_zh)}</span><span class="skill-description-en" lang="en">{_esc(description_en)}</span></p>{_skill_style_line(skill)}<div class="skill-chips">{compatibility}</div>{meta}
      <div class="skill-card-bottom"><button class="skill-details" type="button" data-skill-id="{_esc(skill.get('id'))}"><span lang="zh-CN">查看实测与证据 →</span><span lang="en">Tests &amp; evidence →</span></button>{source}</div>
    </article>'''
...
def _legacy_render_skills_page(skills: list[dict], site_url: str) -> str:
    ...
    * { box-sizing:border-box; } html { scroll-behavior:smooth; } body { margin:0; min-width:320px; background:var(--canvas); color:var(--ink); font:400 15px/1.6 var(--font-sans); -webkit-font-smoothing:antialiased; -moz-osx-font-smoothing:grayscale; } a { color:inherit; } button,input,select { font:inherit; } h1,h2,h3 { font-family:var(--font-serif); font-weight:400; }
    ...
    .freellm-test-strip { margin:10px 0 2px; display:grid; grid-template-columns:auto 1fr auto; align-items:center; gap:9px; padding:9px 10px; border:1px solid var(--line); background:var(--surface-soft); font:600 11px/1.3 var(--font-sans); }
    ...
    .freellm-test-strip.blocked strong { color:#b76a00; } .freellm-test-strip.pending strong { color:var(--ink-tertiary); }
    .freellm-test-panel { border:1px solid var(--line); background:var(--surface-soft); padding:18px; }
    .freellm-test-head { display:flex; gap:12px; justify-content:space-between; align-items:flex-start; margin-bottom:14px; }
    .freellm-test-score { font:800 28px/1 var(--font-mono); color:var(--accent); white-space:nowrap; }
    .freellm-test-meta { display:flex; gap:7px; flex-wrap:wrap; margin-top:7px; }
    .freellm-test-meta span { border:1px solid var(--line); background:var(--surface); padding:3px 7px; border-radius:999px; font:600 10px/1.3 var(--font-mono); color:var(--ink-secondary); }
    .freellm-test-task { margin:0 0 12px; color:var(--ink); line-height:1.65; }
    .freellm-test-verdict { margin:0; padding-top:12px; border-top:1px solid var(--line); color:var(--ink-secondary); line-height:1.7; }
    .freellm-evidence { margin-top:14px; display:grid; gap:10px; }
    .freellm-evidence a { display:block; text-decoration:none; border:1px solid var(--line); background:var(--surface); padding:11px 12px; }
    .freellm-evidence a:hover { border-color:var(--accent); } .freellm-evidence strong { display:block; color:var(--ink); } .freellm-evidence small { color:var(--ink-secondary); }
    .preview-disclaimer { margin:8px 0 0; color:var(--ink-tertiary); font-size:11px; }
    ...
    (() => {
      ...
      const ensurePreview = () => { let section = document.getElementById('dialog-skill-preview'); if (section) return section; section = document.createElement('section'); section.id = 'dialog-skill-preview'; section.className = 'skill-dialog-section skill-preview'; section.innerHTML = '<h3><span lang="zh-CN">输出结构示意（非实测证据）</span><span lang="en">Output structure sketch (not test evidence)</span></h3><p class="preview-disclaimer">真实页面 / 图片 / 文档产物请以上方 FreeLLM 实测证据为准。</p><div class="skill-preview-layout"><div class="skill-preview-copy"><span class="skill-preview-label">OUTPUT FORMAT / 输出形式</span><strong id="dialog-preview-format" class="skill-preview-format"></strong><p id="dialog-preview-summary" class="skill-preview-summary"></p><div id="dialog-preview-tags" class="skill-preview-tags"></div></div><div id="dialog-preview-canvas" class="skill-preview-canvas" role="img" aria-label="Skill output structure preview"></div></div>'; document.getElementById('dialog-skill-stats').after(section); return section; };
      ...
      const renderFreeLLMTest = skill => { const test = skill.freeLLMTest || {}; const section = document.getElementById('dialog-freellm-test'); const score = document.getElementById('dialog-test-score'); const meta = document.getElementById('dialog-test-meta'); const task = document.getElementById('dialog-test-task'); const verdict = document.getElementById('dialog-test-verdict'); const evidence = document.getElementById('dialog-test-evidence'); score.textContent = typeof test.score === 'number' ? test.score.toFixed(1) + '/10' : (test.testLevel === 'preflight' ? 'BLOCKED' : 'UNSCORED'); meta.innerHTML = [test.status, test.testLevel, test.testedAt, test.environment].filter(Boolean).map(item => '<span>' + escapeHtml(item) + '</span>').join(''); task.textContent = test.task || (isEn() ? 'No benchmark task recorded.' : '暂未记录基准任务。'); verdict.textContent = test.evaluation || ''; const items = Array.isArray(test.evidence) ? test.evidence : []; evidence.innerHTML = items.map(item => { const href = typeof item === 'string' ? item : item.url; const label = typeof item === 'string' ? 'Open evidence' : (item.label || 'Open evidence'); const note = typeof item === 'object' && item.note ? item.note : ''; return '<a href="' + escapeHtml(href || '#') + '" target="_blank" rel="noopener"><strong>' + escapeHtml(label) + ' ↗</strong>' + (note ? '<small>' + escapeHtml(note) + '</small>' : '') + '</a>'; }).join(''); evidence.hidden = items.length === 0; section.dataset.level = test.testLevel || ''; };
...
def _merge_skill_tests(skills: list[dict], tests: dict[str, dict]) -> list[dict]:
    """Attach FreeLLM's own test record. Source metadata and third-party reviews stay secondary."""
    if not tests:
        return skills
