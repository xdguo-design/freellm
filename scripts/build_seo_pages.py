...
SKILL_REAL_TEST_REPORT_PATH = "/skills/test-artifacts/real-sandbox-20260920/"
...
def _skill_test_markup(skill: dict) -> str:
    ...
    result_summary = str(test.get("resultSummary") or "")
    evaluation = str(test.get("evaluation") or "")
    limitations = str(test.get("limitations") or "")
    score = test.get("score")
    blocked = level == "preflight"
    real_run = test.get("runKind") == "real-sandbox"
    if isinstance(score, (int, float)):
        lead = f"{score:g}/10"
        state_class = "tested"
        label = f"FreeLLM {'沙箱实测' if real_run else '实测'} · {status}"
    else:
        lead = "BLOCKED" if blocked else "TEST"
        state_class = "blocked" if blocked else "pending"
        label = f"FreeLLM {'沙箱实测 · ' if real_run else ''}{status}"
    evidence_links = []
    ...
    detail = (
        ...
        f'<p><strong>实际结果：</strong>{_esc(result_summary)}</p>'
        f'<p><strong>FreeLLM 评价：</strong>{_esc(evaluation)}</p>'
        f'<p><strong>限制：</strong>{_esc(limitations) if limitations else "无额外限制记录"}</p>'
        f'<p><strong>测试时间：</strong>{_esc(tested_at)} · {_esc(level)} · {"真实沙箱运行" if real_run else "测试记录"}</p>'
...
def _skill_card(skill: dict) -> str:
    ...
    return f'''<article class="skill-card" data-skill-id="{_esc(skill.get('id'))}" data-category="{_esc(skill.get('category'))}" data-status="{_esc(skill.get('status'))}">
      ...
      <div class="skill-card-bottom"><button class="skill-details" type="button" data-skill-id="{_esc(skill.get('id'))}"><span lang="zh-CN">查看实测、SKILL.md 与社区参考 →</span><span lang="en">Test, source &amp; community reference →</span></button>{source}</div>
    </article>'''
def _legacy_render_skills_page(skills: list[dict], site_url: str) -> str:
    ...
    * { box-sizing:border-box; } html { scroll-behavior:smooth; } body { margin:0; min-width:320px; background:var(--canvas); color:var(--ink); font:400 15px/1.6 var(--font-sans); -webkit-font-smoothing:antialiased; -moz-osx-font-smoothing:grayscale; } a { color:inherit; } button,input,select { font:inherit; } h1,h2,h3 { font-family:var(--font-serif); font-weight:400; }
    ...
    .skill-real-test { padding:16px; border:1px solid var(--line); border-radius:10px; background:var(--surface-soft); } .skill-real-test-head { display:flex; justify-content:space-between; gap:12px; align-items:center; } .skill-real-test-head strong { font:700 15px var(--font-mono); color:var(--accent); } .skill-real-test-meta { color:var(--ink-tertiary); font:500 10.5px var(--font-mono); } .skill-real-test dl { display:grid; grid-template-columns:110px 1fr; gap:8px 12px; margin:14px 0 0; } .skill-real-test dt { color:var(--ink-tertiary); font-size:11px; } .skill-real-test dd { margin:0; color:var(--ink); font-size:13px; line-height:1.65; } .skill-real-test .test-report-link { display:inline-flex; margin-top:12px; color:var(--accent); font-size:12.5px; font-weight:700; text-decoration:none; }
    ...
    .skill-content { max-height:300px; overflow:auto; margin:10px 0 0; padding:14px; border-radius:8px; background:var(--code-bg); color:var(--code-text); font:400 11.5px/1.65 var(--font-mono); white-space:pre-wrap; overflow-wrap:anywhere; } .skill-content.is-error { background:var(--pale-yellow-bg); color:var(--pale-yellow-text); }
    .skill-content-translation { margin:0; padding:15px 16px; border:1px solid var(--line); border-radius:8px; background:var(--surface-soft); color:var(--ink); }
    .skill-content-translation strong { display:block; margin-bottom:6px; font-size:12px; color:var(--accent); }
    .skill-content-translation p { margin:0; color:var(--ink); font-size:14px; line-height:1.75; }
    .skill-content-translation .translation-note { margin-top:10px; color:var(--ink-tertiary); font-size:11.5px; }
    .skill-source-original { margin-top:10px; } .skill-source-original > summary { cursor:pointer; width:max-content; color:var(--accent); font-size:12.5px; font-weight:600; }
    .review-original { margin-top:8px; } .review-original > summary { cursor:pointer; color:var(--ink-tertiary); font-size:11px; } .review-original p { margin:6px 0 0; color:var(--ink-secondary); font-size:12px; line-height:1.65; }
    ...
    (() => {
      ...
      const renderReviews = skill => { const items = reviewsOf(skill); const list = document.getElementById('dialog-skill-reviews'); if (!items.length) { list.innerHTML = `<li class="skill-review-item is-empty"><span lang="zh-CN">暂未收录针对该 Skill 的第三方评价；上方 star / fork 数据可作为社区热度参考。</span><span lang="en">No third-party review collected yet; the star / fork counts above serve as a popularity signal.</span></li>`; return; } list.innerHTML = items.map(review => { const sentiment = sentimentLabel(review.sentiment); const translated = !isEn() && review.quote_zh ? review.quote_zh : review.quote; const original = !isEn() && review.quote_zh && review.quote_zh !== review.quote ? `<details class="review-original"><summary>查看英文原文</summary><p>“${escapeHtml(review.quote)}”</p></details>` : ''; return `<li class="skill-review-item"><blockquote>“${escapeHtml(translated)}”</blockquote>${original}<div class="review-meta">${sentiment ? `<span class="skill-sentiment ${sentiment[1]}"><span lang="zh-CN">${sentiment[0]}</span><span lang="en">${sentiment[1]}</span></span>` : ''}${review.date ? `<span>${escapeHtml(review.date)}</span>` : ''}<a href="${escapeHtml(review.url)}" target="_blank" rel="nofollow noopener">${escapeHtml(review.source)} ↗</a></div></li>`; }).join(''); };
      const contentCache = {};
      const renderContent = skill => { const pre = document.getElementById('dialog-skill-content'); const meta = document.getElementById('dialog-content-meta'); const translation = document.getElementById('dialog-content-translation'); const summary = document.getElementById('dialog-content-summary-zh'); const original = document.getElementById('dialog-source-original'); summary.textContent = skill.description_zh || skill.description || '暂无中文说明。'; translation.hidden = isEn(); original.open = isEn(); pre.classList.remove('is-error'); if (!skill.contentPath) { pre.classList.add('is-error'); meta.innerHTML = ''; pre.textContent = isEn() ? 'No SKILL.md could be verified for this entry. Open the GitHub link to inspect the source yourself.' : '该条目未能核验到 SKILL.md 原文（来源链接缺失或不可达），请打开 GitHub 自行核对。'; return; } const show = doc => { pre.textContent = doc.content; meta.innerHTML = `${escapeHtml(String(doc.bytes || ''))} bytes · ${isEn() ? 'fetched ' : '抓取于 '}${escapeHtml(String(doc.fetchedAt || '').slice(0, 10))} · <a href="${escapeHtml(doc.contentUrl || skill.githubUrl || '#')}" target="_blank" rel="nofollow noopener">raw ↗</a>`; }; if (contentCache[skill.id]) { show(contentCache[skill.id]); return; } pre.textContent = isEn() ? 'Loading SKILL.md…' : '正在加载 SKILL.md 原文…'; meta.innerHTML = ''; fetch(`content/${encodeURIComponent(skill.id)}.json`).then(response => { if (!response.ok) throw new Error('http ' + response.status); return response.json(); }).then(doc => { contentCache[skill.id] = doc; if (current && current.id === skill.id) show(doc); }).catch(() => { if (current && current.id === skill.id) { pre.classList.add('is-error'); pre.textContent = isEn() ? 'Failed to load the content. Open GitHub to read the source.' : '原文加载失败，请打开 GitHub 查看源文件。'; } }); };
      const renderRealTest = skill => { const test = skill.freeLLMTest || {}; const section = document.getElementById('dialog-real-test'); if (!test.runKind) { section.hidden = true; return; } section.hidden = false; const score = typeof test.score === 'number' ? test.score.toFixed(1).replace('.0','') + '/10' : (isEn() ? 'Not scored' : '不评分'); document.getElementById('dialog-test-state').textContent = `${test.status || ''} · ${score}`; document.getElementById('dialog-test-meta').textContent = `${test.testedAt || ''} · ${test.testLevel || ''} · ${test.runKind === 'real-sandbox' ? (isEn() ? 'real sandbox run' : '真实沙箱运行') : ''}`; document.getElementById('dialog-test-task').textContent = test.task || ''; document.getElementById('dialog-test-result').textContent = test.resultSummary || ''; document.getElementById('dialog-test-evaluation').textContent = test.evaluation || ''; document.getElementById('dialog-test-limitations').textContent = test.limitations || (isEn() ? 'No additional limitation recorded.' : '无额外限制记录。'); const link = document.getElementById('dialog-test-report'); link.href = `/skills/test-artifacts/real-sandbox-20260920/index.html#${encodeURIComponent(skill.id)}`; };
...
def render_skill_real_test_report(skills: list[dict], site_url: str) -> str:
    """Render the actual 2026-09-20 sandbox run. This page deliberately contains no synthetic previews."""
    rows = []
    counts = {"通过": 0, "部分通过": 0, "环境阻塞": 0}
    for skill in skills:
        test = skill.get("freeLLMTest") or {}
        if test.get("runKind") != "real-sandbox":
            continue
        status = str(test.get("status") or "")
        counts[status] = counts.get(status, 0) + 1
        score = test.get("score")
        score_text = f"{score:g}/10" if isinstance(score, (int, float)) else "不评分"
        evidence = "".join(
            f'<li><a href="{_esc(item.get("url"))}" target="_blank" rel="noopener">{_esc(item.get("label") or "真实产物")} ↗</a></li>'
            for item in (test.get("evidence") or [])
            if isinstance(item, dict) and item.get("url") and not str(item.get("url")).startswith(SKILL_REAL_TEST_REPORT_PATH + "#")
        )
        rows.append(f'''<article class="sandbox-result" id="{_esc(skill.get("id"))}">
          <header><div><div class="eyebrow">{_esc(test.get("testLevel"))} / REAL SANDBOX</div><h2>{_esc(skill.get("name"))}</h2><code>{_esc(skill.get("id"))}</code></div><div class="sandbox-score"><strong>{_esc(status)}</strong><span>{_esc(score_text)}</span></div></header>
          <h3>固定测试任务</h3><p>{_esc(test.get("task"))}</p>
          <h3>实际结果</h3><p>{_esc(test.get("resultSummary"))}</p>
          <h3>FreeLLM 真实评价</h3><p class="evaluation">{_esc(test.get("evaluation"))}</p>
          <h3>限制 / 没有测到什么</h3><p>{_esc(test.get("limitations")) or "无额外限制记录。"}</p>
          {f'<h3>真实产物</h3><ul>{evidence}</ul>' if evidence else ''}
        </article>''')
    page_url = _absolute(site_url, SKILL_REAL_TEST_REPORT_PATH)
    description = f"FreeLLM 2026-09-20 对 {sum(counts.values())} 个 Agent Skill 的真实沙箱测试：{counts.get('通过',0)} 通过、{counts.get('部分通过',0)} 部分通过、{counts.get('环境阻塞',0)} 环境阻塞。"
    return f'''<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>FreeLLM Skills 真实沙箱测试 · 2026-09-20</title><meta name="description" content="{_esc(description)}"><meta name="robots" content="noindex,follow"><link rel="canonical" href="{_esc(page_url)}">{SKILLS_THEME_ASSETS}
<style>{EDITORIAL_BASE_CSS}
.sandbox-hero{{padding:28px;border:1px solid var(--line);border-radius:18px;background:var(--surface)}}.sandbox-summary{{display:flex;gap:9px;flex-wrap:wrap;margin:18px 0}}.sandbox-summary span{{padding:6px 10px;border:1px solid var(--line);border-radius:999px;background:var(--surface-soft)}}.sandbox-result{{margin:18px 0;padding:22px;border:1px solid var(--line);border-radius:14px;background:var(--surface)}}.sandbox-result header{{display:flex;justify-content:space-between;gap:18px;align-items:flex-start}}.sandbox-result h2{{margin:4px 0}}.sandbox-result h3{{margin:18px 0 5px;font-size:12px;color:var(--ink-secondary);letter-spacing:.08em;text-transform:uppercase}}.sandbox-score{{text-align:right}}.sandbox-score strong,.sandbox-score span{{display:block}}.sandbox-score span{{font:600 20px var(--font-mono)}}.evaluation{{font-size:16px;font-weight:600}}@media(max-width:640px){{.sandbox-result header{{display:block}}.sandbox-score{{text-align:left;margin-top:10px}}}}
</style></head><body><main><section class="sandbox-hero"><div class="eyebrow">REAL SANDBOX RUN / 2026-09-20</div><h1>68 个 Skill，逐个真跑</h1><p>{_esc(description)} 不具备外部账号、API Key、网络或指定 CLI 的项目明确标记阻塞，不拿本地替代品冒充通过；视觉类产物实际打开复核后再评分。</p><div class="sandbox-summary"><span>通过 {counts.get('通过',0)}</span><span>部分通过 {counts.get('部分通过',0)}</span><span>环境阻塞 {counts.get('环境阻塞',0)}</span><span>阻塞项不评分</span></div><a href="/skills/">← 返回 Skills</a></section>{''.join(rows)}</main></body></html>'''
...
def _expected_files(offers: list[dict], site_url: str, models: list[dict] | None = None, operations: list[dict] | None = None, daily_logs: list[dict] | None = None, skills: list[dict] | None = None, recipes: list[dict] | None = None, data_dir: Path | None = None, vendor_directory: list[dict] | None = None) -> tuple[dict[Path, str], list[str]]:
    ...
    files: dict[Path, str] = {
        **render_sitemaps(offers, categories, site_url, model_catalog, providers),
        ...
        Path("skills") / "test-artifacts" / "real-sandbox-20260920" / "index.html": render_skill_real_test_report(skills or [], site_url),
